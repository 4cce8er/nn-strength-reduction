...
x_train, y_train, x_test, y_test = Mnist.get_scaled_data()
...
def main():
    ...
    if args.lite:
        ...
        tf_utils.set_dataset_v2(dataset=Mnist)
        tf_utils.convert_to_tflite(repr_dataset=Mnist.representative_data_gen)
    if args.summary:
        print(model.summary())
        # convert_to_tflite(model, args.lite, model_name="")
    save_model(model, model_name=args.name)
